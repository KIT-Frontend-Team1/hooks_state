...
function Q2() {
  ...
  //추가버튼 누를때 forceRender가 변경되는 것을 이용해서, 해당 forceRender
  //가 변경되면 ShowList가 false상태가 되어 리스트가 보이지 않게 함.
  useEffect(() => {
    setShowList(false);
  }, [forceRender]);
...
//   //제출버튼을 누르면 arr에 있던 내용이 list에서 렌더링 되어야 함.
//   const onSubmit = () => {
//     if (arr.current.lenght === 0) return;
//     setText(arr.current.map((item) => <li key={Math.random()}>{item}</li>)); //text : []
//     console.log("submit text", text); //undefined
//   };

//   // const onSubmit = () => {
//   //   return arr.current.map((item) => <li key={Math.random()}>{item}</li>)
//   // }
...
//         {arr.current.length === 0 && text && <p>제출된 목록이 없습니다</p>}
//         {arr.current.length && <ul>{text}</ul>}
//       </div>
//       <div>
//         <h2>문제 2-2</h2>
//         <p ref={targetText}> 이 문구는 아래 버튼을 누르면 색상이 바뀝니다</p>
